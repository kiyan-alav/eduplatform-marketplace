...
export const enrollmentUserService = {
  async getAll(userId: number, filter: GetAllEnrollmentsQuery) {
    const student = await enrollmentUserRepository.findStudentProfileByUserId(userId);
  ...
  async getOne(userId: number, courseId: number) {
    ...
    const enrollment = await enrollmentUserRepository.findById(student.id, courseId);
  ...
  async create(userId: number, data: ICreateEnrollmentRequest) {
    ...
    const existing = await enrollmentUserRepository.findById(student.id, data.courseId);
    if (existing) {
